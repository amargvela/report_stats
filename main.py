...
#        false_negative_threshold = 0.001
        ...
        best_dev_index = translate_from_threshold_to_index(best_result['dev'], best_threshold)
        # print(best_dev_index, len(results[0]['dev']['fpr']), len(best_result['dev']['fpr']))
        best_test_index = translate_from_threshold_to_index(best_result['test'], best_threshold)

        dev_best = 1 - best_result['dev']['fpr'][best_dev_index]
        ...
        num_epochs = results[-1]['epoch'] + 1

        test_best = 1 - best_result['test']['fpr'][best_test_index]
        ...
        # print(best_i)
        best_random_test_index = translate_from_threshold_to_index(results[0]['test'], best_per_epoch[0])
        best_random_dev_index = translate_from_threshold_to_index(results[0]['dev'], best_per_epoch[0])
        test_random = 1 - results[0]['test']['fpr'][best_random_test_index]
        best_random_test = test_random * negative_ratio_in_data + false_negative_threshold * positive_ratio_in_data

        dev_random = 1 - results[0]['dev']['fpr'][best_random_dev_index]
        best_random_dev = dev_random * negative_ratio_in_data + false_negative_threshold * positive_ratio_in_data

        print 'Best % triage dev {:.2f}% for false_negative_threshold {:.1f}% on epoch {:d} out of {:d} epochs'.format(100*best_dev_ratio, 100*false_negative_threshold, best_result['epoch'], num_epochs)
        print 'Best % triage test {:.2f}% for false_negative_threshold {:.1f}% on epoch {:d} out of {:d} epochs'.format(100*best_test_ratio, 100*false_negative_threshold, best_result['epoch'], num_epochs)
        ...
        best_test_result = best_result['test']
        print 'Test accuracy for best negative_ratio_by_model {:.2f}%'.format(100*best_test_result['accuracy'])
        print 'confusion matrix for best negative_ratio_by_model'
        print np.array(best_test_result['confusion_matrix'])



def translate_from_threshold_to_index(result, target_threshold):
        closest_threshold_index = -1
        best_threshold_distance = 100
        for i in range(len(result['thresholds'])):
                result_threshold = result['thresholds'][i]
...
def translate_from_threshold2_to_index(result, target_threshold):
        closest_threshold_index = -1
        best_threshold_distance = 100
        for i in range(len(result['thresholds2'])):
                result_threshold = result['thresholds2'][i]
                result_threshold_distance = (result_threshold - target_threshold)**2.0
                if result_threshold_distance < best_threshold_distance and target_threshold > result_threshold:
                        closest_threshold_index = i
                        best_threshold_distance = result_threshold_distance
